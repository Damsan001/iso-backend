...
usuario_rol = Table(
    ...
    Column(
        "usuario_id",
        BigInteger,
        ForeignKey(f"{SCHEMA_NAME}.usuario.usuario_id", ondelete="CASCADE"),
        primary_key=True,
    ),
    Column(
        "rol_id",
        BigInteger,
        ForeignKey(f"{SCHEMA_NAME}.rol.rol_id", ondelete="CASCADE"),
        primary_key=True,
    ),
    schema=SCHEMA_NAME,
)
...
class CatalogItem(Base):
    ...
    catalog_id = Column(
        BigInteger,
        ForeignKey(f"{SCHEMA_NAME}.catalog.catalog_id", ondelete="CASCADE"),
        nullable=False,
    )
    empresa_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.empresa.empresa_id"), nullable=True
    )
    code = Column(Text)
    name = Column(Text, nullable=False)
    description = Column(Text)
    ...
    parent_item_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.catalog_item.item_id"), nullable=True
    )
    created_at = Column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    )
...
class Activo(Base):
    ...
    empresa_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.empresa.empresa_id"), nullable=False
    )
    nombre = Column(Text, nullable=False)
    tipo_item_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.catalog_item.item_id"), nullable=False
    )
    ...
    propietario_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.usuario.usuario_id"), nullable=True
    )
    custodio_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.usuario.usuario_id"), nullable=True
    )
...
class ComentarioRevision(Base):
    ...
    version_id = Column(
        BigInteger,
        ForeignKey(f"{SCHEMA_NAME}.documento_version.version_id", ondelete="CASCADE"),
        nullable=False,
    )
    usuario_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.usuario.usuario_id"), nullable=False
    )
    comentario = Column(Text, nullable=False)
    archivo_url = Column(Text)
    fecha = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    estatus_item_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.catalog_item.item_id"), nullable=True
    )
    deleted_at = Column(DateTime(timezone=True))
...
class Documento(Base):
    ...
    area_responsable_item_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.catalog_item.item_id"), nullable=True
    )
    clasificacion_item_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.catalog_item.item_id"), nullable=True
    )
    visible_empresa = Column(
...
class DocumentoACL(Base):
    ...
    documento_id = Column(
        BigInteger,
        ForeignKey(f"{SCHEMA_NAME}.documento.documento_id", ondelete="CASCADE"),
        nullable=False,
    )
    empresa_id = Column(
        BigInteger, ForeignKey(f"{SCHEMA_NAME}.empresa.empresa_id"), nullable=True
...
class Usuario(Base):
    ...
    __table_args__ = (
        Index(
            "uq_usuario_empresa_email", "empresa_id", text("lower(email)"), unique=True
        ),
        {"schema": SCHEMA_NAME},
    )
    ...
    empresa_id = Column(
        BigInteger,
        ForeignKey(f"{SCHEMA_NAME}.empresa.empresa_id", ondelete="CASCADE"),
        nullable=False,
    )
    area_id = Column(
        BigInteger,
        ForeignKey(f"{SCHEMA_NAME}.areas.area_id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    roles = relationship(
        "Rol", secondary=f"{SCHEMA_NAME}.usuario_rol", back_populates="usuarios"
    )
...
class Rol(Base):
    ...
    __table_args__ = (
        Index(
            "uq_rol_empresa_nombre", "empresa_id", text("lower(nombre)"), unique=True
        ),
        {"schema": SCHEMA_NAME},
    )
    ...
    usuarios = relationship(
        "Usuario", secondary=f"{SCHEMA_NAME}.usuario_rol", back_populates="roles"
    )
    permisos = relationship(
        "Permiso", secondary=f"{SCHEMA_NAME}.rol_permiso", backref="roles"
    )
...
class RolPermiso(Base):
    ...
    permiso_id = Column(
        BigInteger,
        ForeignKey(f"{SCHEMA_NAME}.permiso.permiso_id", ondelete="CASCADE"),
        primary_key=True,
    )
...
class Areas(Base):
    ...
    __table_args__ = (
        Index(
            "uq_areas_empresa_nombre", "empresa_id", text("lower(nombre)"), unique=True
        ),
        {"schema": SCHEMA_NAME},
    )
