...
async def login_for_access_token(
    ...
    token = create_access_token(
        payload=payload, expires_delta=datetime.timedelta(minutes=60)
    )
...
async def read_users_me(user: user_dependency, db: db_dependency):
    ...
    return {"usuario": usuarios_dto, "roles": roles, "permisos": permisos}


@router.put("/activate", status_code=status.HTTP_200_OK)
async def activate_user_account(
    db: db_dependency, user: user_dependency, request: ActivateUserRequest
):
...
def reset_password(request: ResetPasswordRequest, db: Session = Depends(get_db)):
    resp = reset_password_service(
        request.token, request.new_password, request.confirm_password, db
    )
    return resp
