...
from app.schemas.risks_schema import (
    ...
    RiesgoGeneralUpdate,
    RiesgoGeneralOut,
    RiesgoGeneralListPage,
    RiesgoActivoCreate,
    RiesgoActivoUpdate,
    RiesgoActivoOut,
    RiesgoActivoListPage,
)
...
def crear_riesgo_general(
    payload: RiesgoGeneralCreate, db: db_dependency, user: user_dependency
):
    return svc.create_riesgo_general(db, user, payload)


@router.patch("/generales/{riesgo_id}", response_model=RiesgoGeneralOut)
def actualizar_riesgo_general(
    riesgo_id: int,
    payload: RiesgoGeneralUpdate,
    db: db_dependency,
    user: user_dependency,
):
...
def crear_riesgo_activo(
    payload: RiesgoActivoCreate, db: db_dependency, user: user_dependency
):
    return svc.create_riesgo_activo(db, user, payload)


@router.patch("/activos/{riesgo_id}", response_model=RiesgoActivoOut)
def actualizar_riesgo_activo(
    riesgo_id: int,
    payload: RiesgoActivoUpdate,
    db: db_dependency,
    user: user_dependency,
):
...
@router.get(
    "/catalogos/probabilidad",
    response_model=list[CatalogoItemSimple],
    summary="Catálogo Probabilidad",
)
...
@router.get(
    "/catalogos/nivel-riesgo",
    response_model=list[CatalogoItemSimple],
    summary="Catálogo Nivel de Riesgo",
)
