...
from app.services.admin_service import (
    ...
    delete_role_service,
    patch_role_service,
    add_roles_to_user_service,
    get_roles_by_user_service,
    remove_roles_from_user_service,
    get_permissions_by_role_service,
    read_all_permissions,
    read_permission_by_name,
    create_permission_service,
    patch_permission_service,
    update_permission_service,
    add_permissions_to_user_service,
    remove_permissions_from_user_service,
    get_permissions_by_user_service,
    delete_permission_service,
    add_permissions_to_role_service,
    remove_permissions_from_role_service,
)
...
async def create_role(
    db: db_dependency, user: user_dependency, role_data: CreateRoleDTO
):
...
async def update_role(
    role_id: int, db: db_dependency, user: user_dependency, role_data: UpdateRoleDTO
):
...
async def assign_roles_to_user(
    db: db_dependency, user: user_dependency, user_id: int, role_ids: list[int]
):
    add_roles_to_user_service(db, user, user_id, role_ids)
    return {"message": "Roles asignados al usuario exitosamente"}
...
async def unassign_roles_from_user(
    ...
    return {"message": "Roles removidos del usuario exitosamente"}
...
async def get_permiso_by_name(
    permiso_name: str, db: db_dependency, user: user_dependency
):
...
async def create_permiso(
    db: db_dependency, user: user_dependency, permiso_data: CreatePermissionDTO
):
...
async def update_permiso(
    ...
    permiso_data: UpdatePermissionDTO,
):
...
async def assign_permisos_to_user(
    db: db_dependency, user: user_dependency, user_id: int, permiso_ids: list[int]
):
    add_permissions_to_user_service(db, user, user_id, permiso_ids)
    return {"message": "Permisos asignados al usuario exitosamente"}


@router.post("/permisos/unassign/{user_id}", status_code=status.HTTP_200_OK)
async def unassign_permisos_from_user(
    db: db_dependency, user: user_dependency, user_id: int, permiso_ids: list[int]
):
    remove_permissions_from_user_service(db, user, user_id, permiso_ids)
    return {"message": "Permisos removidos del usuario exitosamente"}


@router.post("/permisos/assign/role/{role_id}", status_code=status.HTTP_200_OK)
async def assign_permisos_to_role(
    db: db_dependency, user: user_dependency, role_id: int, permiso_ids: list[int]
):
...
async def unassign_permisos_from_role(
    db: db_dependency, user: user_dependency, role_id: int, permiso_ids: list[int]
):
    remove_permissions_from_role_service(db, user, role_id, permiso_ids)
    return {"message": "Permisos removidos del rol exitosamente"}
