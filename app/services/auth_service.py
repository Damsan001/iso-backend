...
def create_access_token(
    *, payload: dict, expires_delta: timedelta | None = timedelta(minutes=15)
) -> str:
...
def obtener_roles_usuario(db: Session, usuario_id: int) -> list[str]:
    roles = (
        ...
        .filter(usuario_rol.c.usuario_id == usuario_id, Rol.activo.is_(True))
        .all()
    )
...
def obtener_permisos_usuario(db: Session, usuario_id: int) -> list[str]:
    permisos = (
        ...
        .filter(
            ...
            & (UsuarioPermiso.concedido.is_(True))
...
def reset_password_service(
    token: str, new_password: str, confirm_password: str, db: Session
):
...
def forgot_password_service(
    email: str, db: Session, background_tasks: BackgroundTasks, url_site: str
):
    user = db.query(Usuario).filter(Usuario.email == email).first()
    if not user:
        return {"mensaje": "Si el correo existe, se enviará un enlace de recuperación."}
    token = create_access_token(
        payload={"sub": str(user.usuario_id), "email": user.email},
