...
from app.infrastructure.models import Rol, Empresa, Usuario, AuditLog, Permiso, UsuarioPermiso
from app.services.auth_service import check_auth_and_roles
from app.schemas.Dtos.AdminDtos import RoleDTO, CreateRoleDTO, UpdateRoleDTO, PermissionDTO, CreatePermissionDTO, UpdatePermissionDTO
...
def create_role_service(db: Session, user: dict, role_data: CreateRoleDTO):
    ...
    existing = db.query(Rol).filter(
        ...
        Rol.empresa_id == user.get("empresa_id")
    ).first()
    if existing:
        raise HTTPException(
...
def add_roles_to_user_service(db: Session, user: dict, user_id: int, role_ids: list[int]):
    check_auth_and_roles(user, ["Administrador"])
...
def remove_roles_from_user_service(db: Session, user: dict, user_id: int, role_ids: list[int]):
    check_auth_and_roles(user, ["Administrador"])
    ...
    roles_to_remove = db.query(Rol).filter(Rol.rol_id.in_(role_ids), Rol.activo == True).all()
...
def read_permission_by_name(db: Session, user: dict, permission_name: str):
    ...
    if not permisos:
        return None
    return [PermissionDTO.model_validate(p) for p in permisos]
def create_permission_service(db: Session, user: dict, permission_data: CreatePermissionDTO):
    check_auth_and_roles(user, ["Administrador"])
...
def update_permission_service(db: Session, user: dict, permission_id: int, permission_data: CreatePermissionDTO):
    check_auth_and_roles(user, ["Administrador"])

    permission = (db.query(Permiso)
...
def patch_permission_service(db: Session, user: dict, permission_id: int, permission_data: UpdatePermissionDTO):
    check_auth_and_roles(user, ["Administrador"])
...
def add_permissions_to_user_service(db: Session, user: dict, user_id: int, permission_ids: list[int]):
    check_auth_and_roles(user, ["Administrador"])
    ...
    permisos = db.query(Permiso).filter(Permiso.permiso_id.in_(permission_ids), Permiso.activo == True).all()
    if not permisos:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    for permiso in permisos:
        existe = db.query(UsuarioPermiso).filter_by(usuario_id=user_id, permiso_id=permiso.permiso_id).first()
        if not existe:
            up = UsuarioPermiso(usuario_id=user_id, permiso_id=permiso.permiso_id, concedido=True)
            db.add(up)
...
def remove_permissions_from_user_service(db: Session, user: dict, user_id: int, permission_ids: list[int]):
    check_auth_and_roles(user, ["Administrador"])
    ...
    permisos_to_remove = db.query(Permiso).filter(Permiso.permiso_id.in_(permission_ids), Permiso.activo == True).all()
...
def get_permissions_by_user_service(db: Session, user: dict, user_id: int):
    ...
    return [PermissionDTO.model_validate(p) for p in usuario.permisos_directos if p.activo]


def add_permissions_to_role_service(db: Session, user: dict, role_id: int, permission_ids: list[int]):
    check_auth_and_roles(user, ["Administrador"])
    ...
    permisos = db.query(Permiso).filter(Permiso.permiso_id.in_(permission_ids), Permiso.activo == True).all()
    if not permisos:
        raise HTTPException(status_code=404, detail="No se encontraron permisos válidos")
...
def remove_permissions_from_role_service(db: Session, user: dict, role_id: int, permission_ids: list[int]):
    check_auth_and_roles(user, ["Administrador"])
    rol = db.query(Rol).filter(Rol.rol_id == role_id, Rol.activo == True).first()
    if not rol:
        raise HTTPException(status_code=404, detail="Rol no encontrado")
    ...
    missing = set(permission_ids) - found_permission_ids
    if missing:
        raise HTTPException(status_code=404, detail=f"Los siguientes permisos no existen o no están activos: {list(missing)}")
...
def get_permissions_by_role_service(db: Session, user: dict, role_id: int):
    ...
    return [PermissionDTO.model_validate(permiso) for permiso in rol.permisos if permiso.activo]
# endregion
