...
class ReportService:
    ...
    def generate_audit_report_excel(filters: AuditFilter) -> BytesIO:
        ...
        def in_range(d):
            return filters.date_from <= d.created_at <= filters.date_to

        def type_ok(d):
            return not filters.types or d.type in filters.types

        def area_ok(d):
            return not filters.areas or d.area_responsible in filters.areas
        ...
        df = pd.DataFrame(
            rows,
            columns=["id", "code", "name", "type", "area", "version", "created_at"],
        )
        buffer = BytesIO()
